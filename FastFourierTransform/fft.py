...
def FFT2D_shift(fft):
    ...
    for i in range(rows):
        for j in range(cols):
            index = (cols/2 + j) % cols
            # pdb.set_trace()
            tmp[i, int(index)] = fft[i, j]

    for j in range(cols):
        for i in range(rows):
            index = (rows/2 + i) % rows
            ret[int(index), j] = tmp[i, j]
...
if __name__ == '__main__':
    ...
    filename = "Q1.tif" if not args.filename else args.filename
    ...
    misc.imsave("saqibImages/%s_unshifted_fft.png" % filename.split('.')[0], spectrum)
    ...
    misc.imsave("saqibImages/%s_shifted_fft.png" % filename.split('.')[0], spectrum)
